...
NUM_HOURS = 24
NUM_DAYS = 7                                        #gpu 1 badal 2 line 292 
NUM_WEEKS = 52                                      # multi gpu

#class ModelMGPU(Model):
   # def __init__(self, ser_model, gpu): #gpus
    #    pmodel = (ser_model, gpu) #multi_gpu_model FEL AWEL --- gpus
...
def construct_mlp(input_size, num_classes, num_frames,
                  dropout_size=0.5, ef_mode=3, l2_reg=1e-5):
    """
    ...
    # Add hidden layers
    from keras.layers import Flatten, Conv1D, Conv2D, GlobalMaxPooling1D, GlobalAveragePooling1D, LSTM, Concatenate, GlobalAveragePooling2D,GlobalMaxPooling2D, LeakyReLU
    ...
    if ef_mode == 0:
        ...
        base_model = efn.EfficientNetB3(weights='imagenet', include_top=False, pooling='max')
    elif ef_mode == 4:
        base_model = efn.EfficientNetB4(weights='imagenet', include_top=False, pooling='avg')  #imagenet or weights='noisy-student'
    elif ef_mode == 5:
...
def train_model(base_model, training_generator, validation_generator, output_dir,
   ...
   # model = ModelMGPU(base_model, gpu=1) #2 #,gpus=1---
    base_model.compile(Adam(lr=learning_rate), loss=loss)

    history = base_model.fit_generator(generator=training_generator, validation_data=validation_generator,# steps_per_epoch=846,
...
def train(annotation_path, taxonomy_path, emb_dir, output_dir, exp_id,
          ...
          ef_mode=3, l2_reg=1e-5, standardize=True,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ef_mode", type=int, default=3)
